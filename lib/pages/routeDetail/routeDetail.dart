...
import 'package:gtlmd/pages/routeDetail/routeDetailViewModel.dart';
...
class _RoutedetailState extends State<Routedetail> {
    ...
    viewModel.routeAcceptLiveData.stream.listen((resp) {
      if (resp.commandstatus == 1) {
        setState(() {
          ...
          // showSuccessAlert(context, "PLANNING ACCEPTED SUCCESSFULLY", "",
          //     okayCallBackForAlert);
          getRouteDetails();
        });
      } else {
    ...
    viewModel.routeRejectLiveData.stream.listen((resp) {
      if (resp.commandstatus == 1) {
        setState(() {
          ...
          okayCallBackForAlert();
          // showSuccessAlert(context, "PLANNING REJECTED SUCCESSFULLY", "",
          //     okayCallBackForAlert);
        });
      } else {
        failToast(resp.commandmessage.toString() ?? "Something went wrong");
  ...
  void updateRoute() {
    ...
    // String grno = routeDetailList.map((r) => r.grno ?? '').join(',');
    // String address = routeDetailList.map((r) => r.address ?? '').join(',');
    // String deliverylat =
    //     routeDetailList.map((r) => r.deliverylat?.toString() ?? '').join(',');
    // String deliverylong =
    //     routeDetailList.map((r) => r.deliverylong?.toString() ?? '').join(',');
    // String pickuplat =
    //     routeDetailList.map((r) => r.pickuplat?.toString() ?? '').join(',');
    // String pickuplong =
    //     routeDetailList.map((r) => r.pickuplong?.toString() ?? '').join(',');
    // String distance =
    //     routeDetailList.map((r) => r.distance?.toString() ?? '0').join(',');
    // String sequenceid =
    //     routeDetailList.map((r) => r.sequenceid?.toString() ?? '').join(',');
    // String draftcode =
    //     routeDetailList.map((r) => r.planningdraftcode ?? '').join(',');
    ...
    try {
      ...
      if (totalDistance >
        ...
        showAlert("Distance Increased", const Icon(Icons.update_outlined), "",
          ...
          double.parse(modelDetail.totdistance!
              .substring(0, modelDetail.totdistance!.length - 4))) {
        // double diff = double.parse(modelDetail.totdistance!
        //         .substring(0, modelDetail.totdistance!.length - 4)) -
        //     totalDistance;
        // double diff2 = double.parse(diff.toStringAsFixed(2));

        // showAlert("Distance Reduced", const Icon(Icons.update_outlined), "",
        //     "Your distance has reduced by $diff2 km. Do you still want to update?",
        //     () {
        //   setState(() {
        //     updateSequenceBeforeSubmit(totalDistanceList);
        //   });
 ...
 */
                  ...
                  routeDetailList.isEmpty
                      ...
                      //         color:
                      //             CommonColors.dangerColor!.withOpacity(0.2)),
                      //     child: Text(
