...
class ReminderListRepository extends BaseRepository {
  ...
  Future<void> archieveReminder(Map<String, String> params) async {
    viewDialog.add(true);
    final hasInternet = await NetworkStatusService().hasConnection;

    if (hasInternet) {
      CommonResponse resp =
          await apiGet("${bookingUrl}archieveReminders", params);
      viewDialog.add(false);
      ...
      if (resp.commandStatus == 1) {
        ...
        for (final entry in entries) {
          if (entry.key == "Table1") {
            List<dynamic> list1 = entry.value;
            List<CommonUpdateModel> resultList = List.generate(list1.length,
                (index) => CommonUpdateModel.fromJson(list1[index]));
            archieveLiveData.add(resultList[0]);
          }
        }
      } else {
        isErrorLiveData.add(resp.commandMessage!);
